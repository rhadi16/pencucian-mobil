...
if (strlen($_POST["password"])<8) {echo '<script language="javascript"> window.location.href = "../operator.php?desc=short-pass" </script>';break;}
...
if ($exists) {echo '<script language="javascript"> window.location.href = "../operator.php?desc=email-ready" </script>';break;}
...
try {
$sql = $db->prepare("INSERT INTO utenti (email, password, nama, posisi) VALUES (? ,?, ?, ?)");
$sql->bindParam(1, $_POST["email"]);
$sql->bindParam(2, $hash);
$sql->bindParam(3, $_POST["nama"]);
$sql->bindParam(4, $_POST["posisi"]);
$sql->execute();
...
$registered=1;
echo '<script language="javascript"> window.location.href = "../operator.php?desc=success-in" </script>';
